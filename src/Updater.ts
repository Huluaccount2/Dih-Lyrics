...
export class Updater {
    ...
    public static async forceUpdate(): Promise<void> {
        ...
        await Updater.downloadRepo("Huluaccount2", "Dih-Lyrics", "v3", downloadPath)

        // Replaces files pointing to the correct extracted folder structure: Dih-Lyrics-v3
        Updater.replaceFiles(join(downloadPath, "./Dih-Lyrics-v3"), join(__dirname, "../"), exclude)
    }

    public static async checkUpdate(): Promise<boolean> {
        // Checks your fork's raw VERSION file on the v3 branch for changes
        const version = await (await fetch("https://github.com/Huluaccount2/Dih-Lyrics/raw/refs/heads/v3/VERSION")).text()
