...
export class QQMusicSource extends BaseSource {
    private async request(url: string): Promise<Response> {
        return fetch(url, {
            headers: { "Referer": "http://y.qq.com/portal/player.html" }
        })
    }

    private async getSongId(name: string, artist: string): Promise<string> {
    ...
    private parseLyrics(lyrics: string): SongLyrics {
        ...
        for (const line of lines) {
            ...
            result.lines.push({ time: (60 * m + s) * 1000 + ms, text: decode(text) })
        }
