...
interface StyleEntry {
    loOffset: number
    hiOffset: number
    exceptions: Record<number, string> | null
...
export const FONT_STYLES: string[] = ["none", ...Object.keys(STYLES), "underline", "strikethrough"]

function shift(text: string, entry: StyleEntry): string {
    return [...text].map((ch) => {
        const cp = ch.codePointAt(0)

        if (cp === undefined) return ch
        if (entry.exceptions && cp in entry.exceptions) return entry.exceptions[cp]
        if (cp >= 0x61 && cp <= 0x7A) return String.fromCodePoint(cp + entry.loOffset) // a-z
        if (cp >= 0x41 && cp <= 0x5A) return String.fromCodePoint(cp + entry.hiOffset) // A-Z
        if (entry.digitOffset != null && cp >= 0x30 && cp <= 0x39) return String.fromCodePoint(cp + entry.digitOffset) // 0-9
...
export function applyFontStyle(text: string, style: string): string {
    if (!text || !style || style === "none") return text
    if (style === "underline") return underline(text)
    if (style === "strikethrough") return strikethrough(text)
